...
위쪽 값은 `0x61fe14`로 뜨지만, 아래쪽 값은 `0x61fe18`로 뜬다. 이는 포인터 변수에 값을 더하면 해당 타입의 크기만큼 더해진다. int형은 4바이트이기 때문에 4바이트만큼 더해진다.

이렇게 동작하는 이유는 포인터 산술의 동작 방식 때문이다. 이를 위해 포인터와 메모리의 연관성을 다시한번 생각해야 하는데, 포인터는 메모리 주소를 저장하는 변수이다. C++에서 포인터는 특정 타입의 변수를 가리키며, 이 타입에 따라 포인터가 가리키는 메모리 블록의 크기도 결정되게 된다.

`int` 타입의 포인터는 4바이트 크기의 정수를 가리키고 있으며, 이는 `int` 타입의 변수 하나의 크기와 같다. 따라서 포인터에 1을 더하면 4바이트만큼 이동하게 된다. 즉 실제로는 포인터가 가리키는 자료형의 sizeof(type)에 `n`을 곱한 만큼 이동하게 된다.
